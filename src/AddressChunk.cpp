...
namespace memory{
	void AddressChunk::clear(){
		...
		_next = (AddressChunk*)WASM_NULL_PTR;
	}
	...
	bool AddressChunk::remove(size_t idx) {
		...
		if (_len == ADDRESSCHUNK_LEN) {
			if (_next != WASM_NULL_PTR && _next->_len > 0) {
	...
	std::ostream &operator<<(std::ostream &stream, const AddressChunk &chunk){
		stream << "length   " << std::hex << std::setw(2) << std::setfill('0') << chunk._len << std::endl;

		for (size_t i = 0; i < chunk._len; ++i){
			stream << std::hex << std::setw(2) << std::setfill('0') << i << "       " << chunk._addr[i] << std::endl;
		}
